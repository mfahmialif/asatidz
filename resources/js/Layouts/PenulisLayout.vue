<template>
 <div :data-theme="isDark ? 'dark' : 'light'" class="portal-root relative flex h-screen w-screen font-display overflow-hidden transition-colors duration-500">
  ...
  <div class="hidden lg:block relative shrink-0 transition-all duration-300" :style="{ width: sidebarCollapsed ? '72px' : '256px' }">
   <PenulisSidebar :collapsed="sidebarCollapsed" class="h-full" @close-sidebar="sidebarOpen = false" />
  ...
  <PenulisSidebar :collapsed="false" :class="['fixed z-40 h-screen lg:hidden transition-transform duration-300', sidebarOpen ? 'translate-x-0' : '-translate-x-full']" style="width: 256px" @close-sidebar="sidebarOpen = false" />

  <main class="flex min-w-0 flex-1 flex-col h-screen overflow-hidden" :style="{ background: 'var(--bg-main)' }">
   <PenulisNavbar :page-title="pageTitle" :is-dark="isDark" @toggle-theme="toggleTheme" @toggle-sidebar="sidebarOpen = !sidebarOpen" />
...
import PenulisSidebar from '../components/layouts/penulis/PenulisSidebar.vue'
import PenulisNavbar from '../components/layouts/penulis/PenulisNavbar.vue'
import { readThemePreference, resolveThemePreference, writeThemePreference } from '../composables/useThemePreference'

const route = useRoute()
const pageTitle = computed(() => route.meta.pageTitle || 'Dashboard Penulis')
const sidebarOpen = ref(false)
const sidebarCollapsed = ref(false)
const isDark = ref(true)

onMounted(() => {
 sidebarCollapsed.value = localStorage.getItem('penulis-sidebar-collapsed') === 'true'
 const savedTheme = readThemePreference({
  allowSystem: true,
  fallback: 'system',
  legacyKeys: ['penulis-theme'],
 })
...
function toggleCollapse() {
 ...
 localStorage.setItem('penulis-sidebar-collapsed', String(sidebarCollapsed.value))
}
