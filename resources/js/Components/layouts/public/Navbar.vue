<template>
 <header class="fixed inset-x-0 top-0 z-50 px-4 pt-4 sm:px-6 lg:pt-6">
  <div class="public-nav-shell mx-auto flex max-w-7xl items-center justify-between gap-4 rounded-[1.7rem] px-5 py-3 backdrop-blur-xl lg:px-7">
   <Link href="/" class="logo-pill group" :class="{ 'logo-pill-image': showLogoImage }">
    ...
    <span v-if="!showLogoImage" class="leading-tight">
     <span class="block text-base font-black text-white">{{ brand.title || page.props.app_settings?.system_name || 'Asatidz' }}</span>
     <span class="block text-[10px] font-black uppercase tracking-wide text-sky-300">{{ brand.subtitle || page.props.app_settings?.navbar_text_subtitle || 'Program Studi' }}</span>
    </span>
   </Link>
   ...
   <nav v-else class="public-nav-links hidden items-center gap-2 text-[14px] font-bold md:flex">
    <div v-for="item in items" :key="item.href" class="nav-item-wrap">
     ...
     <Link v-else :href="item.href" class="nav-link" :class="{ active: isActive(item) }">
      <span>{{ item.label }}</span>
      <span v-if="item.children?.length" class="nav-branch-arrow material-symbols-outlined" aria-hidden="true">keyboard_arrow_down</span>
     </Link>
   ...
   <div class="hidden items-center gap-2 md:flex">
    ...
    <Link :href="authLink.to" class="public-signin hidden h-9 items-center gap-2 rounded-full px-3.5 text-sm font-black transition md:inline-flex">
   ...
   <div v-if="mobileOpen" class="public-mobile-panel mx-auto mt-3 max-w-7xl rounded-3xl p-4 shadow-2xl backdrop-blur-xl md:hidden">
    ...
    <div v-for="item in loading ? [] : items" :key="mobileItemKey(item)" class="mobile-menu-group">
     <div class="mobile-menu-row">
      ...
      <Link v-else :href="item.href" @click="closeMobileMenu" class="public-mobile-link flex min-w-0 flex-1 items-center rounded-2xl px-4 py-3 font-bold" :class="{ active: isActive(item) }">
       <span class="truncate">{{ item.label }}</span>
      </Link>
      <button
     ...
     <div v-if="item.children?.length && isMobileExpanded(item)" class="mobile-submenu-list">
      ...
      >
       ...
       <Link v-else :href="child.href" @click="closeMobileMenu" class="public-mobile-link flex min-w-0 flex-1 items-center rounded-2xl px-4 py-2.5 text-sm font-bold">
        <span class="truncate">{{ child.label }}</span>
       </Link>
       <button
    ...
    <Link :href="authLink.to" class="public-mobile-signin mt-2 flex items-center justify-center gap-2 rounded-2xl px-4 py-3 font-black" @click="mobileOpen = false">
