<template>
 <aside :class="['sidebar flex h-screen flex-col p-4 shrink-0 transition-all duration-300', collapsed ? 'sidebar-collapsed' : '']">
  <div class="flex items-center px-3 pb-4 shrink-0" :class="collapsed ? 'justify-center' : 'justify-between'">
   <Link href="/" class="brand-link overflow-hidden transition-all duration-300" :style="collapsed ? 'width: 0; opacity: 0' : 'width: auto; opacity: 1'">
  ...
  <div class="flex-1 min-h-0 overflow-y-auto custom-scrollbar">
    <nav class="flex flex-col gap-1 pr-1 pb-4">
     <template v-for="section in navSections" :key="section.label">
      ...
      <Link v-for="item in section.items" :key="item.label" :href="item.route"
             :class="[
...
const navSections = [
 {
  label: '',
  items: [
   { icon: 'dashboard', label: 'Dashboard', route: '/asatidz/dashboard' },
  ],
 },
 {
  label: 'Kegiatanku',
...
.custom-scrollbar::-webkit-scrollbar-thumb { background: var(--border-light); border-radius: 999px; opacity: 0; transition: background 0.4s ease; }
.sidebar:hover .custom-scrollbar::-webkit-scrollbar-thumb { background: var(--text-muted); }
...
.brand-link, .brand-icon-link { display: block; text-decoration: none; }
.brand-link:hover .text-heading, .brand-icon-link:hover { color: var(--color-accent); }
.brand-icon-link { display: inline-flex; align-items: center; justify-content: center; color: var(--color-accent); }
.nav-item { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); border-left: 0 solid transparent; position: relative; }
.nav-item:hover { background: var(--hover-nav-bg); color: var(--color-accent); transform: translateX(4px); }
.nav-item:active { transform: translateX(2px) scale(0.98); }
.section-heading:first-child { margin-top: 0; }
.section-heading.is-collapsed { height: 1px; margin: 18px 12px 10px; padding: 0; background: var(--border); }
.section-heading.is-collapsed:first-child { display: none; }
.sidebar-collapsed .sidebar-label { width: 0; opacity: 0; overflow: hidden; white-space: nowrap; transition: width 0.3s ease, opacity 0.2s ease; }
.sidebar-label { transition: width 0.3s ease, opacity 0.2s ease 0.1s; white-space: nowrap; }
.sidebar-collapsed .sidebar-link { justify-content: center; gap: 0; padding: 8px; width: 44px; height: 44px; margin: 0 auto; border-radius: 12px; }
