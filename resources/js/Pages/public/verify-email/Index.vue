 ...
 <main class="auth-page min-h-screen" :data-theme="resolvedTheme">
  ...
  <section class="flex min-h-screen items-center justify-center px-5 py-10">
   <div class="auth-card w-full max-w-[460px] rounded-2xl p-6 shadow-2xl sm:p-8">
    <Link href="/register" class="auth-back mb-7 inline-flex items-center gap-2 text-sm font-black">
...
const email = ref(new URLSearchParams(window.location.search).get('email') || '')
const code = ref('')
...
onMounted(() => {
 initTheme()
 if (!email.value) router.visit('/register')
})
</script>
