   ...
   <div class="table-wrapper rounded-xl overflow-hidden shadow-2xl">
    <div class="overflow-x-auto p-2">
     <table class="w-full text-left border-collapse">
      <thead>
       <tr class="table-head">
        <th class="px-6 py-4 text-sm font-semibold tracking-wide" style="color: var(--text-heading)">Nama Kegiatan</th>
        <th class="px-6 py-4 text-sm font-semibold tracking-wide" style="color: var(--text-heading)">Poin</th>
      ...
      <tbody class="table-body">
       <tr v-for="kegiatan in store.kegiatans" :key="kegiatan.id" class="table-row-hover">
        <td class="px-6 py-5">
         <span class="text-sm font-medium" style="color: var(--text-heading)">{{ kegiatan.nama }}</span>
        </td>
        <td class="px-6 py-5">
         <span class="admin-badge admin-badge--sky">{{ kegiatan.poin }}</span>
        </td>
        <td class="px-6 py-5">
         <span :class="statusBadge(kegiatan.status)">{{ kegiatan.status }}</span>
        </td>
        <td class="px-6 py-5 text-right">
         <div class="flex items-center justify-end gap-1">
          <button @click="openEditModal(kegiatan)" class="action-btn p-2 rounded-lg transition-all duration-200" title="Edit">
           <span class="material-symbols-outlined text-[20px]">edit</span>
          </button>
          <button @click="confirmDelete(kegiatan)" class="action-btn action-btn-delete p-2 rounded-lg transition-all duration-200" title="Delete">
    ...
    <div class="pagination-bar flex items-center justify-between px-6 py-4">
     <span class="text-sm font-medium" style="color: var(--text-muted)">Menampilkan {{ store.pagination.from || 0 }} ke {{ store.pagination.to || 0 }} dari {{ store.pagination.total || 0 }} kegiatan</span>
  ...
  <Transition name="fade">
   <div v-if="showModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="showModal = false">
    <div class="modal-card w-full max-w-lg rounded-2xl p-6 flex flex-col gap-5">
     <h3 class="text-lg font-bold" style="color: var(--text-heading)">{{ editingData ? 'Edit Kegiatan' : 'Tambah Kegiatan' }}</h3>
     <div class="grid grid-cols-1 gap-4">
      <div class="flex flex-col gap-1.5">
       <label class="text-sm font-medium" style="color: var(--text-body)">Nama Kegiatan</label>
       <input v-model="form.nama" class="filter-input rounded-xl py-2.5 px-4 text-sm focus:outline-none focus:ring-1 focus:ring-accent" placeholder="Nama Kegiatan" />
      </div>
      <div class="flex flex-col gap-1.5">
       <label class="text-sm font-medium" style="color: var(--text-body)">Poin</label>
       <input v-model="form.poin" type="number" min="1" class="filter-input rounded-xl py-2.5 px-4 text-sm focus:outline-none focus:ring-1 focus:ring-accent" placeholder="Poin" />
      </div>
      <div class="flex flex-col gap-1.5">
...
import { useMasterKegiatanStore } from '../../../stores/masterKegiatan'

const store = useMasterKegiatanStore()
...
const form = ref({ nama: '', poin: 1, status: 'Aktif' })
const formError = ref('')
...
function openCreateModal() {
 ...
 form.value = { nama: '', poin: 1, status: 'Aktif' }
 formError.value = ''
 showModal.value = true
}

function openEditModal(kegiatan) {
 editingData.value = kegiatan
 form.value = {
  nama: kegiatan.nama,
  poin: kegiatan.poin,
...
async function confirmDelete(kegiatan) {
 if (!confirm(`Hapus kegiatan "${kegiatan.nama}"?`)) return
 try {
  await store.deleteKegiatan(kegiatan.id)
  loadData()
 } catch (e) {
...
function statusBadge(status) {
 ...
 if (status === 'Aktif') return `${base} admin-badge--success`
