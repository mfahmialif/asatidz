  ...
  <form class="form-card rounded-2xl p-6" @submit.prevent="submit">
   ...
   <div class="grid gap-4">
    ...
    <label class="field">
     ...
     <small class="field-help">Untuk menambahkan route bawaan seperti <code>/news</code> atau <code>/contact</code>, pilih <strong>Custom URL</strong>.</small>
    </label>
    <label v-if="form.type === 'post'" class="field">
  ...
  <section class="rounded-2xl p-2" style="background: var(--bg-card); border: 1px solid var(--border)">
   <div class="flex items-center justify-between gap-4 p-4">
    <div>
     ...
     <p class="text-sm" style="color: var(--text-muted)">Atur menu publik dari Posts atau custom URL.</p>
    </div>
...
function fillLabelFromReference() {
 ...
 const found = options.posts.find((item) => Number(item.id) === Number(form.value.reference_id))
