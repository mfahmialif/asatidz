...
class SpaSeoController extends Controller
{
 ...
 {
  $siteName = AppSetting::getValue('system_name', config('app.name'));
  $landing = $this->landingContent();
  $defaultDescription = $landing['hero']['description'] ?? 'Website resmi Program Studi Doktor Pendidikan Bahasa Arab untuk informasi akademik, berita, kegiatan mahasiswa, dan layanan komunikasi prodi.';
  ...
  if ($path === '/news') {
   return array_replace($meta, [
    'title' => 'News - ' . $siteName,
    'description' => $landing['news']['description'] ?? 'Berita, kegiatan, dan informasi terbaru Program Studi Doktor Pendidikan Bahasa Arab.',
   ]);
  }
